...
class Client(object):
    ...
    def _validate_limit(limit):
        ...
        if limit > max_limit:
            raise ValueError(
                "Limit {} more than max limit: {}".format(limit, max_limit)
            )
        if limit not in valid_limits:
            raise ValueError(
                "Limit {} not among acceptable values: {}".format(limit, valid_limits)
            )
    ...
    def _get_params_with_signature(self, **kwargs):
        ...
        sign = hmac.new(
            self.api_secret, bytes(body, "utf-8"), hashlib.sha256
        ).hexdigest()
        return {"signature": sign, **kwargs}

    def _get_header(self, **kwargs):
        return {**kwargs, CurrencyComConstants.HEADER_API_KEY_NAME: self.api_key}
    ...
    def get_agg_trades(
        self, symbol, start_time: datetime = None, end_time: datetime = None, limit=500
    ):
        """
        ...
        if limit > CurrencyComConstants.AGG_TRADES_MAX_LIMIT:
            raise ValueError(
                "Limit should not exceed {}".format(
        ...
        r = requests.get(
            CurrencyComConstants.AGGREGATE_TRADE_LIST_ENDPOINT, params=params
        )
    ...
    def get_klines(
        ...
        if end_time:
            params["endTime"] = self._to_epoch_miliseconds(end_time)
        r = requests.get(CurrencyComConstants.KLINES_DATA_ENDPOINT, params=params)
        return r.json()
    ...
    def get_account_trade_list(
        ...
        r = self._get(CurrencyComConstants.ACCOUNT_TRADE_LIST_ENDPOINT, **params)
    ...
    def list_leverage_trades(self, recv_window=None):
        ...
        self._validate_recv_window(recv_window)
        r = self._get(
            CurrencyComConstants.TRADING_POSITIONS_ENDPOINT, recvWindow=recv_window
        )
        return r.json()
