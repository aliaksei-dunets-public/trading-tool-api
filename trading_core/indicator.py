...
class Indicator_ATR(IndicatorBase):
    """A class for the Average True Range (ATR) technical indicator"""

    def __init__(self, length: int):
        """
        Initialize the ATR indicator with a specific length.
        """
        # Initialize the base class
        IndicatorBase.__init__(self)
        # Set the code and name of the indicator
        self._code = IndicatorType.ATR.name
        self._name = "Average True Range"

        # Set the length of the indicator (number of periods)
        self.__length = int(length)

    def get_length(self) -> int:
        """Return the length of the ATR indicator"""
        return self.__length

    def get_indicator(self, param: IndicatorParamModel) -> pd.DataFrame:
        """
        Get the ATR indicator for a specific symbol, interval, and time period.
        """
        # Calculate the default limit to ensure there is enough data to calculate the indicator
        default_length = self.__length + 2
    ...
    def get_indicator_by_history_data(
        ...
        """
        Get the ATR indicator for a specific historical data object.
        """
        # Get the historical data as a pandas DataFrame
        history_data = super().get_indicator_by_history_data(history_data_mdl)
        ...
        # Calculate the ATR using the length specified in the constructor
        atr_series = history_data.ta.atr(length=self.__length)

        # Convert the series to a DataFrame with the indicator code as the column name
        atr_df = atr_series.to_frame(name=self._code)

        # Join the indicator DataFrame with the historical data DataFrame
        indicator_df = history_data.join(atr_df)
...
class Indicator_CCI_ATR(IndicatorBase):
    """A class for the CCI and ATR technical indicators"""

    def __init__(self, length: int):
        """
        Initialize the CCI and ATR indicators with a specific length.
        """
        # Initialize the base class
        IndicatorBase.__init__(self)
        # Set the code and name of the indicator
        self._code = IndicatorType.CCI_ATR.name
        self._name = "Commodity Channel Index and Average True Range"

        # Set the length of the indicator (number of periods)
        self.__length = int(length)

    def get_length(self) -> int:
        """Return the length of the CCI and ATR indicators"""
        return self.__length

    def get_indicator(self, param: IndicatorParamModel) -> pd.DataFrame:
        """
        Get the indicators for a specific symbol, interval, and time period.
        """
        # Calculate the default limit to ensure there is enough data to calculate the indicator
        default_length = self.__length + 2
    ...
    def get_indicator_by_history_data(
        ...
        """
        Get the indicators for a specific historical data object.
        """
        # Get the historical data as a pandas DataFrame
        history_data = super().get_indicator_by_history_data(history_data_mdl)
        ...
        # Convert the series to a DataFrame with the indicator code as the column name
        cci_df = cci_series.to_frame(name=IndicatorType.CCI.value)

        # Calculate the ATR using the length specified in the constructor
        atr_series = history_data.ta.atr(length=14)

        # Convert the series to a DataFrame with the indicator code as the column name
        atr_df = atr_series.to_frame(name=IndicatorType.ATR.value)

        # Join the indicator DataFrame with the historical data DataFrame
        indicator_df = history_data.join(cci_df)

        # Join the indicator DataFrame with the historical data DataFrame
        indicator_df = indicator_df.join(atr_df)

        # Drop rows with missing values (NaNs) in the indicator column
        indicator_df = indicator_df.dropna(
            subset=[IndicatorType.CCI.value, IndicatorType.ATR.value]
        )
